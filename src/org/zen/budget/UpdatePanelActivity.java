...
public class UpdatePanelActivity extends PanelActivity {
	...
	public void onCreate( final Bundle savedInstanceState) {
		...
		((TextView)findViewById( R.id.tv_panel_amtsign)).setText( amt >= 0.0f ? "$+" : "$-");
	}
	...
	public float checkAction() {
		...
		final float str_amt = Float.parseFloat( ((EditText)findViewById( R.id.et_panel_amt)).getText().toString());
	...
	@Override
	public float exAction() {
		getContentResolver().delete( ContentUris.withAppendedId( BudgetProvider.CONTENT_URI, id), BudgetProvider._ID + "=?",
			new String[] { "" + id});
		return 0.0f;
	}
}
